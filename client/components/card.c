...
void card_display(int x, int y, const char *image_path, const char *name,
                  const char *price) {

  Rectangle card_rect = {
      .x = x, .y = y, .width = CARD_WIDTH, .height = CARD_HEIGHT};
  Color transparent_black = {.r = 0, .g = 0, .b = 0, .a = 175};
  Rectangle name_price_rect = {card_rect.x, card_rect.y + 200, card_rect.width,
                               20};
